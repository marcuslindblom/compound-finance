 ...
 */
import { searchTicker, getPriceHistory, estimateMarketCap, getCompanySizeCategory, } from "./yahoo-finance.js";
const ROLE_SCORES = {
...
function getRoleScore(role) {
    ...
    if (roleLower.includes("ekonomichef") || roleLower.includes("cfo"))
        return 2.5;
    if (roleLower.includes("ordförande"))
        return 2;
    if (roleLower.includes("styrelseledamot"))
        return 1.5;
    if (roleLower.includes("ledande"))
        return 1.5;
    // Close associate or unknown role
    return 0.5;
}
function getSizeSignal(trade) {
    ...
    if (totalValue >= 5_000_000)
        return 2; // Huge — very strong signal
 ...
 */
function getClusterSignal(trade, allTradesForIssuer) {
    ...
    if (uniqueBuyers.size >= 3)
        return 2; // Strong cluster
    if (uniqueBuyers.size >= 2)
        return 1; // Moderate cluster
    return 0;
}
/**
 ...
 */
export async function scoreTrade(trade, allTradesForIssuer = [], options = {}) {
    ...
    const clusterSignal = getClusterSignal(trade, allTradesForIssuer.length > 0 ? allTradesForIssuer : [trade]);
    let contraTrend = 0;
...
function buildReasoning(trade, ctx) {
    const parts = [];
    if (!ctx.isBuy) {
        parts.push(`⚠️ Sell signal — generally weaker (people sell for many reasons)`);
    }
 ...
 */
async function enrichWithStockData(trade) {
    ...
    let contraTrend = 0;
    if (history.changePercent30d < -10)
        contraTrend = 2;
    else if (history.changePercent30d < -5)
        contraTrend = 1;
    ...
    try {
        ...
        if (marketCapSEK > 0) {
            ...
            if (size === "micro")
                ...
                companySizeBonus = 0.75;
            else if (size === "mid")
                companySizeBonus = 0.5;
            // large = 0
        }
    }
    catch {
 ...
 */
export async function scoreAndRank(trades, options = {}) {
    ...
    const scored = await Promise.all(trades.map((t) => scoreTrade(t, byIssuer.get(t.issuer) || [], options)));
